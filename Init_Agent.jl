...
function Create_Agent(start, goal, δt, v_des, α, σ, ϵ, l, γ)

    steps = Int(floor(T(start, goal, v_des)/δt))

    agent(start, start, goal, fill((0.0, 0.0), steps), α, σ, ϵ, δt, v_des, l, γ)

end

function Create_Agent(start, im_goal, goal, δt, v_des, α, σ, ϵ, l, γ)
...
function Init_Parameter!(a::agent, δt, v_des, α, σ, ϵ, γ, l, start, goal)
    a.δt = δt
    a.v_des = v_des
    a.α = α
    a.σ = σ
...
function Init_Initial_Guess_Corner!(a::agent, im_goal)
    ...
    for (i,x) in enumerate(a.traj[1:N2])
        a.traj[i] = a.start .+ d(a.start, im_goal)/T(a)*(i*a.δt) .* e_(a.start, im_goal)
    end

    for (i,x) in enumerate(a.traj[N2+1:end])
        a.traj[N2+i] = im_goal .+ d(im_goal, a.goal)/T(a)*(i*a.δt) .* e_(im_goal, a.goal)
    end
