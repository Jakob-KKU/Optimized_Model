...
#Optimization Algorithm: site by site from start to goal
function Run_Ordered_Optimization!(a::agent, crwd::crowd, obstacle::geometry, steps::Int, Δsteps::Int)

    trajectories = fill((0.0, 0.0), Int(round(steps/Δsteps)+1), length(a.traj))
    energies = fill(0.0, steps+1)

    energies[1] = E(a, crwd, obstacle)
    trajectories[1, :] .= a.traj
    j = 2

    for i in 1:steps

        Update_Ordered!(a, crwd, obstacle)
        ...
        energies[i+1] = E(a, crwd, obstacle)
...
function Run_Ordered_Optimization!(a::agent, crwd, obstacle::geometry, steps::Int, Δsteps::Int, ΔE)
...
function Update_Ordered!(a::agent, crwd::crowd, obstacle::geometry)

    Update_ϵ!(a, crwd, obstacle)

    for site in 1:length(a.traj)

        Update_at!(site, a, crwd, obstacle)

    end

end

function Update_at!(site::Int, a::agent, crwd::crowd, obstacle::geometry)

    a.traj[site] = a.traj[site] .- a.γ .* ∇E(a, site, crwd, obstacle)

end

function Update_ϵ!(a::agent, crwd::crowd, obstacle::geometry)

    a.ϵ += -a.γ*∇E_ϵ(a, crwd, obstacle)
